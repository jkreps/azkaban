...
public class ScheduleManagerTest
{
	  ...
	  public void testLoadScheduleRegular() {
		  ...
		  ArrayList<ExecutionRecord> test1Results = filterByName(executionRecords, "test1");
		  ArrayList<ExecutionRecord> test2Results = filterByName(executionRecords, "test2");
		  ArrayList<ExecutionRecord> test3Results = filterByName(executionRecords, "test3");
		  ArrayList<ExecutionRecord> test4Results = filterByName(executionRecords, "test4");
		  ArrayList<ExecutionRecord> test5Results = filterByName(executionRecords, "test5");
		  ArrayList<ExecutionRecord> test6Results = filterByName(executionRecords, "test6");
	  ...
	  private DateTime[] createCheckList(DateTime startTime, long repeating, DateTime endTime) {
	  ...
	  private ArrayList<ExecutionRecord> filterByName( ArrayList<ExecutionRecord> executionRecords, String name) {
	  ...
	  private void checkTimes(List<ExecutionRecord> executionRecords, DateTime[] times) {
		  Assert.assertEquals("Sizes are the same as expected", executionRecords.size(), times.length);
		  
		  for (int i = 0; i < executionRecords.size(); ++i) {
			  Assert.assertTrue("Unexpected execution", areTimesEqual(executionRecords.get(i).getStartTime(), times[i], 100));
		  }
	  }
	  
	  private boolean areTimesEqual(DateTime a, DateTime b, long msThreshold) {
	  ...
	  public void testLoadRemoveSchedule() {
		  ...
		  checkTimes(test2Results, new DateTime[] { time2 } );
		  checkTimes(test3Results, createCheckList(time3, period3Ms, endTime) );
		  checkTimes(test4Results, createCheckList(time4, period4Ms, baseTime.plus(3000)) );
